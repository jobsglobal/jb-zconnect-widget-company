...
$templateCache.put("{widgetsPath}/jb-zconnect-widget-company/src/templates/insights.html","<div class={{insights.config.class}} style=\"height: {{insights.config.height}}px !important; margin-bootom:15px;\"><nvd3 options=insights.options data=insights.data></nvd3><div class=\"col-lg-12 col-sm-12 col-xs-12\"><div class=row><button type=button class=c_all_cpanel data-ui-sref={{insights.config.linkSref}}><i class=\"fa fa-chevron-circle-right\" aria-hidden=true></i>{{insights.config.linkText}}</button></div></div></div>");
...
$templateCache.put("{widgetsPath}/jb-zconnect-widget-company/src/templates/link.html","<div><a href=javascript: data-ui-sref={{link.config.sref}}><img data-ng-src={{link.config.imageSource}} data-ng-if=\"link.config.imageType==\'image\'\"><i data-ng-if=\"link.config.imageType==\'icon\'\" class={{link.config.imageSource}}></i>{{link.config.text}}</a></div>");
